...
// A session:start-style JWT refresh dance: on a 401 that isn't the refresh
// call itself, swap the access token once and retry the original request.
// Concurrent 401s share a single in-flight refresh so we don't hammer
// POST /auth/refresh once per failed request.
